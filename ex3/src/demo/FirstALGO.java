...
public class FirstALGO  {
	...
	public FirstALGO(String path,String output,int k) {
	...
	public FirstALGO(String path,String output) {
		...
		//System.out.println(s.TypeCSVcheck(path));
		if(s.TypeCSVcheck(path)==2) {
		readFile(path);// making files to objects
		macList();//making mac list
		String str="algo 1 results\n num,MAC,LAT,LON,ALT \n";
		for (int i = 0; i < ALLmac.size(); i++) {
			Macfiltering temp=new Macfiltering(xRead,ALLmac.get(i));		
			double []arr=pointSum1(temp.getTop4sampels());
	...
	public ArrayList<LineRead> gettREAD() {
		return tREAD;
	}
	public void settREAD(ArrayList<LineRead> tREAD) {
		this.tREAD = tREAD;
	}
	public ArrayList<String> getALLmac() {
		return ALLmac;
	}
	public void setALLmac(ArrayList<String> aLLmac) {
		ALLmac = aLLmac;
	}
	public ArrayList<Vector> getAbc() {
		return abc;
	}
	public void setAbc(ArrayList<Vector> abc) {
		this.abc = abc;
	}
